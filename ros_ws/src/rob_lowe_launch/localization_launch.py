...
def generate_launch_description():
    ...
    occup_grid = LaunchConfiguration('occup_grid')
    ...
    declare_slam_params_file_cmd = DeclareLaunchArgument(
        'slam_params_file',
        default_value=os.path.join(get_package_share_directory("rob_lowe_launch"),
                                   'config', 'localization_params.yaml'),
        description='Full path to the ROS2 parameters file to use for the slam_toolbox node')
    declare_starter_map_cmd = DeclareLaunchArgument(
        'occup_grid',
        default_value=os.path.join(get_package_share_directory("rob_lowe_launch"),
                                   'include', 'occup_grid.yaml'),
        description='Path to occupancy grid')
