...
void ROSWitmotionSensorController::imu_process(
  ...
  switch (static_cast<witmotion_packet_id>(packet.id_byte)) {
	  ...
	  case pidOrientation:
	    ...
	    break;
	  default:
		//RCLCPP_INFO(rclcpp::get_logger("ROSWitmotionSensorController"), "no case");
	    return;
  }
//RCLCPP_INFO(rclcpp::get_logger("ROSWitmotionSensorController"), "%d %d %d - %d %d %d", imu_enable_accel, imu_enable_velocities, imu_enable_orientation, imu_have_accel, imu_have_velocities, imu_have_orientation);
...
//RCLCPP_INFO(rclcpp::get_logger("ROSWitmotionSensorController"), "publish");
...
void ROSWitmotionSensorController::Packet(const witmotion_datapacket &packet) {
   //RCLCPP_INFO(rclcpp::get_logger("ROSWitmotionSensorController"), "Packet ID 0x%X acquired", packet.id_byte);
  switch (static_cast<witmotion_packet_id>(packet.id_byte)) {
  case pidRTC:
    rtc_process(packet);
    break;
